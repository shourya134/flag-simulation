...
class Model:
    ...
    def wind_forces(self, time):
        ...
        return forces * self.wind_magnitude * (noise.pnoise1(time) + 0.2) * (
                    noise.pnoise1(noise.pnoise1(time)) * 0.5 + 0.5)
    ...
    def generate_plane(width, height, MAX_WIDTH_SIZE=500, MAX_HEIGHT_SIZE=300, flag_type=default_flag_type):
        ...
        for point in range(n):
            ...
            if point % width == width - 1:
                if point < n - 1:
                    Model.add_spring_const(verts, point, point + width, constraints)
                continue
            ...
            if point >= n - width and point < n:
                Model.add_spring_const(verts, point, point + 1, constraints)

        for i in range(height):
            fixed_point = width * i
            Model.add_fixed_constraint(n, fixed_point, verts[fixed_point], constraints)
    ...
    def add_face(v_1, v_2, v_3, faces, neighbours):
        neighbours[v_1].append(v_2);
        neighbours[v_1].append(v_3)
        neighbours[v_2].append(v_1);
        neighbours[v_2].append(v_3)
        neighbours[v_3].append(v_2);
        neighbours[v_3].append(v_1)
    ...
    def add_spring_constraint_set(verts, v_1, v_2, v_3, constraints):
        Model.add_spring_const(verts, v_1, v_2, constraints)
        Model.add_spring_const(verts, v_2, v_3, constraints)
        Model.add_spring_const(verts, v_3, v_1, constraints)

    def add_spring_const(verts, v_1, v_2, constraints):
